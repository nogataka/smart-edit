...
const INTELEPHENSE_DEPENDENCIES: RuntimeDependency[] = [
  {
    id: 'intelephense',
...
function ensureIntelephenseBinary(
  runtimeDir: string,
  ...
  if (process.env.SMART_EDIT_SKIP_RUNTIME_INSTALL === '1') {
    throw new Error(
      `Intelephense executable not found at ${binaryPath}. Allow downloads by unsetting SMART_EDIT_SKIP_RUNTIME_INSTALL or install manually.`
    );
  }
  ...
  if (!fs.existsSync(binaryPath)) {
    throw new Error(
      `Intelephense installation completed but executable missing at ${binaryPath}. Verify npm installation succeeded.`
    );
  }
  ...
  private registerHandlers(): void {
    ...
    this.handler.onNotification('window/logMessage', (payload) => {
      if (payload && typeof payload === 'object' && 'message' in (payload as Record<string, unknown>)) {
        this.logger.info(`intelephense: ${(payload as { message?: string }).message ?? ''}`);
      }
    });
...
registerLanguageServer(Language.PHP, IntelephenseLanguageServer as SmartLanguageServerConstructor);
